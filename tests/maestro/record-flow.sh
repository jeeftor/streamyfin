...
load_env_file() {
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    case "$key" in
      MAESTRO_APP_ID|MAESTRO_SERVER_URL|MAESTRO_USERNAME|MAESTRO_PASSWORD|MAESTRO_CF_ACCESS_CLIENT_ID|MAESTRO_CF_ACCESS_CLIENT_SECRET|MAESTRO_PLATFORM|MAESTRO_TARGET|MAESTRO_DEVICE) ;;
      *) continue ;;
    esac
...
case "$FLOW_NAME" in
  simple)
    require_vars "$FLOW_NAME" MAESTRO_APP_ID MAESTRO_SERVER_URL MAESTRO_USERNAME
    ;;
  cf|cf-simple)
    require_vars "$FLOW_NAME" MAESTRO_APP_ID MAESTRO_SERVER_URL MAESTRO_USERNAME MAESTRO_CF_ACCESS_CLIENT_ID MAESTRO_CF_ACCESS_CLIENT_SECRET
    ;;
esac
