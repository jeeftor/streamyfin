...
usage() {
  cat >&2 <<EOF
Usage: sh tests/maestro/run-flow.sh simple|cf|ios-simple|ios-cf|tv-simple|play-steamboat-willie|ios-play-steamboat-willie|tv-play-steamboat-willie
...
load_env_file() {
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    case "$key" in
      MAESTRO_APP_ID|MAESTRO_SERVER_URL|MAESTRO_USERNAME|MAESTRO_PASSWORD|MAESTRO_CF_ACCESS_CLIENT_ID|MAESTRO_CF_ACCESS_CLIENT_SECRET|MAESTRO_PLATFORM|MAESTRO_TARGET|MAESTRO_DEVICE|MAESTRO_CLEAR_CLIPBOARD|MAESTRO_MOVIES_LIBRARY_ID|MAESTRO_SHOWS_LIBRARY_ID|MAESTRO_MUSIC_LIBRARY_ID|MAESTRO_STEAMBOAT_WILLIE_ID|MAESTRO_BIG_BUCK_BUNNY_ID) ;;
      *) continue ;;
    esac
...
adb_start_tv_player() {
  if ! command -v adb >/dev/null 2>&1; then
    die "adb is required for Android TV player navigation"
  fi

  item_id=${MAESTRO_STEAMBOAT_WILLIE_ID#media-item-}
  player_url="streamyfin://player/direct-player?itemId=$item_id"
  adb_shell am start -W -a android.intent.action.VIEW -d "$player_url" "$MAESTRO_APP_ID" >/dev/null
}
...
run_android_simple_flow() {
  artifact_dir=$1
  server_url_for_android=${MAESTRO_SERVER_URL#http://}
...
case "${1-}" in
  simple|cf|ios-simple|ios-cf|tv-simple|play-steamboat-willie|ios-play-steamboat-willie|tv-play-steamboat-willie) selected_flow=$1 ;;
  *) usage; exit 2 ;;
esac
...
if [ "$MAESTRO_PLATFORM" = "android" ] && [ "${MAESTRO_SERVER_URL:-}" = "http://localhost:8096" ]; then
...
case "$MAESTRO_PLATFORM:${MAESTRO_SERVER_URL:-}" in
  ios:http://10.0.2.2:*)
    printf '⚠️  WARNING: Using Android emulator host URL with iOS selected.\n'
    printf '   Consider setting MAESTRO_SERVER_URL=http://localhost:8096\n'
    printf '   Or run: make jellyfin-configure-urls MAESTRO_TARGET=ios\n\n'
    ;;
esac
...
case "$selected_flow" in
  ...
  play-steamboat-willie|ios-play-steamboat-willie|tv-play-steamboat-willie)
    if [ "$selected_flow" = "ios-play-steamboat-willie" ]; then
      require_vars "$selected_flow" MAESTRO_APP_ID MAESTRO_SERVER_URL MAESTRO_USERNAME MAESTRO_MOVIES_LIBRARY_ID MAESTRO_STEAMBOAT_WILLIE_ID
    else
